...
public class PlayerTest
{
    ...
    public IEnumerator SellCard_WorkCorrectly_WhenCalled()
    {
        ...
        player.CardArray.Value[0] = card1;
        gameStats.Gold.Value = 0;

        player.SellCard(0);
        Assert.AreEqual(2, gameStats.Gold.Value);
        Assert.Null(player.CardArray.Value[0]);

        var card2 = new Card(Card.CardSuit.Clubs, 0);
        card2.Price = 8;
        player.CardArray.Value[1] = card2;
        player.SellCard(1);
        Assert.AreEqual(6, gameStats.Gold.Value);
        Assert.Null(player.CardArray.Value[1]);

        Assert.Throws<IndexOutOfRangeException>(() => player.SellCard(player.CardArray.Value.Count()));
    }
}
